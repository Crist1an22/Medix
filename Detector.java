...
public class Detector {
    ...
    public void detectarRostros(String ruta, JLabel label) {
        ...
        // Verificar si el archivo Haar Cascade existe antes de cargarlo
        String cascadeFile = "C:/OPENCV/opencv/sources/data/haarcascades/haarcascade_frontalface_default.xml";

        // Verifica que el archivo exista
        File file = new File(cascadeFile);
        if (!file.exists()) {
            System.out.println("El archivo Haar Cascade no se encuentra en la ruta especificada.");
        } else {
            ...
            if (faceDetector.empty()) {
                System.out.println("Error al cargar el archivo Haar Cascade.");
            } else {
                System.out.println("Archivo Haar Cascade cargado con éxito.");
            }
        }
    }
    ...
    // // Determina el tipo de BufferedImage basado en el número de canales de la
    // imagen
    // int bufferedImageType = image.channels() > 1 ? BufferedImage.TYPE_3BYTE_BGR :
    // BufferedImage.TYPE_BYTE_GRAY;
    //
    // // Crea un nuevo objeto BufferedImage con el tamaño de la imagen
    // BufferedImage bufferedImage = new BufferedImage(image.cols(), image.rows(),
    // bufferedImageType);
    //
    ...
    // bufferedImage.getRaster().setDataElements(0, 0, image.cols(), image.rows(),
    // data);
    //
    ...
    public String buscarImagen() {
        ...
        return this.rutaImagen; // Retorna la ruta de la imagen seleccionada, o una cadena vacía si no se
